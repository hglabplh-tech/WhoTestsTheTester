...
\end{flushleft}
\subsection{Functional / Performance / Stress testing as example}
\begin{tikzpicture}
%% fun tests circle
\draw  [fill=blue](0,0) circle  (150pt);
\draw  [fill=cyan](0,0) circle  (40pt);
%the nodes
\node [fill=green] (blackCenter) {\abbrhighcol{Black Box \newline Types}};
\node [fill=olive,left=of  blackCenter] (blackLeft) { \color{white}Functional Testing};
\node [fill=olive,right=of blackCenter] (blackRight) {\color{white}Regression Tests};
\node [fill=olive,above=of  blackCenter] (blackAbove) { \color{white}Performance Testing};
\node [fill=olive,below=of blackCenter] (blackBelow) {\color{white}Stress Tests};
%drawings of lines
...
\begin{enumerate}
\item{Functional / Performance / Stress testing as example for definition}
\item{UNIT testing definition}
\item{Interactive testing definition}
\item{Security testing definition}
\end{enumerate}
