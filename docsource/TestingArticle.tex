...
\newpage
\chapter{The test types}
For categorising and describing the test types I used also input from \hyperref[ref:funnonfun]{Solutions Hub -- see web link}
\section{Test types in overview}
\subsection{Functional test-types in overview}
\begin{tikzpicture}
%% fun tests circle
\draw  [fill=yellow](0,0) circle  (150pt);
\draw  [fill=cyan](0,0) circle  (40pt);
%the nodes
\node [fill=red] (funCenter) { \color{white}\abbrhighcol{Functional Test \newline Types}};
\node [fill=blue,left=of funCenter] (funLeft) { \color{green}Black Box Tests};
\node [fill=blue,right=of funCenter] (funRight) { \color{yellow}White Box Tests};
\node [fill=green,above=of funCenter] (funAbove) {Grey Box Tests};
\node [fill=green,below=of funCenter] (funBelow) {UNIT Tests};
\node [fill=olive,left=of funBelow] (funBelowLeft) {\color{white}Regression Tests};
\node [fill=olive,right=of funBelow] (funBelowRight) {\color{white}Re-Test Tests};
\node [fill=cyan,above=of funAbove] (funAboveAbove) {UAT};
\node [fill=pink,left=of funAbove] (funAboveLeft) {Smoke};
\node [fill=pink,right=of funAbove] (funAboveRight) {Database};
\node [fill=purple,right=of funAboveAbove] (funAboveARight) {\color{white}Recovery};
\node [fill=purple,left=of funAboveAbove] (funAboveALeft) {\color{white}Static};
\node [fill=cyan,below=of funBelow] (funBelowBelow) {Acceptance};
\node [fill=magenta,left=of funBelowBelow] (funBelowBLeft) {\color{white}Integration};
\node [fill=magenta,right=of funBelowBelow] (funBelowBRight) {\color{white}Component};
\node [fill=black,below=of funBelowBelow] (funBelowBBelow) {\color{green}Ad hoc/Random};
\node [fill=black,above=of funAboveAbove] (funAboveAAbove) {\color{green}System Testing};
%drawings of lines
...
\end{tikzpicture}
\subsection{NON-Functional test-types in overview}
\begin{tikzpicture}
%% fun tests circle
\draw  [fill=green](0,0) circle  (150pt);
\draw  [fill=orange](0,0) circle  (40pt);
%the nodes
\node [fill=blue] (nfunCenter) { \color{white}\abbrhighcol{NON-Functional Test \newline Types}};
\node [fill=pink,left=of nfunCenter] (nfunLeft) {Performance Test};
\node [fill=pink,right=of nfunCenter] (nfunRight) {PenetrationTests / Security Testing};
\node [fill=magenta,above=of nfunCenter] (nfunAbove) {\color{white}Upgrade / Migration Testing};
\node [fill=magenta,below=of nfunCenter] (nfunBelow) {\color{white}Reliability Testing};
\node [fill=orange,above=of nfunAbove] (nfunAboveAbove) {Usability Testing};
\node [fill=orange,below=of nfunBelow] (nfunBelowBelow) {Recovery Testing};
\node [fill=cyan,left=of nfunAbove] (nfunAboveLeft) {Compatibility Testing};
\node [fill=cyan,right=of nfunBelow] (nfunBelowRight) {Stress Testing};
%drawings of lines
...
\end{tikzpicture}
\newpage
\section{Test types as description}
...
\draw[->] (mA2.west) .. controls +(down:11mm) and +(left:11mm) .. (mA.west);
...
\newpage
\subsection{The automation between the tools and inside the tools}
\chapter{Clean code and coding rules to make testing easier}
...
The output calling it with all parameters will be:\newline
 ...
 \section{References}
  \label{ref:funnonfun}
  %make it to a list and change label to be a reference on the section..... and use enumerate so that the whole thing can be referenced through the file 
  \hspace*{1em}-- \href{https://about.gitlab.com/ebook-ultimate-guide-ci-cd/variant/?utm_medium=cpc&utm_source=google&utm_campaign=eg_emea_dmp_x_x_en_gitlab_search_nb_singleappci-usecase_emea_broad&utm_content=eg_global_cmp_gated-content_depflex_en_guidecicd_x_x&&utm_term=cicd&_bt=740261293420&_bk=cicd&_bm=b&_bn=g&_bg=145507599496&gad_source=1&gad_campaignid=18463394777&gbraid=0AAAAADcJCbdc_52VE3ELeOTJbyqTR_PEd&gclid=CjwKCAiAlMHIBhAcEiwAZhZBUuxNCoQbO-WCuXyx0-26PE-PWqhcrsxolCDW-4gZq7TBt2eGK9hbvBoCgaYQAvD_BwE}{GITLAB CD/CI Guide and descriptions}\\
\hspace*{1em}-- \href{https://solutionshub.epam.com/blog/post/the-difference-between-functional-and-non-functional-testing}{Definition of Functional and NON Functional Testing}\\
\hspace*{1em}-- \href{https://solutionshub.epam.com/blog/post/regression-testing-test-io}{Regression Testing Tool and Description Regression Test IO}  
\end{document}