...
\title{About Testing, Automation and Project Management for High Quality Projects \newline Version 0.8}

\author{Harald Glab-Plhak{\textless}\href{mailto:hglabplhak@icloud.com}{hglabplhak@icloud.com}{\textgreater}}
\date{\today{}}
%______________________________________________________________________________________
\hypersetup{
pdftitle={About Testing, Automation and Project Management for High Quality Projects Version 0.8}, 
...
\maketitle
\subtitle{Experiences / Theories and practical tips and models for High Quality Projects....}
\tableofcontents
...
After that we can think about how test driven development can be used to have a well defined interface and to code an application. Test driven development is a kind of development where we define in opposite to the prior example the tests first. Test driven development has the advantage that we see immediately if the function we designed works like it should work because the test shows already up the call and return value of a function how we designed it. Instead of adapting the test to our functionality we write the functionality in the way that we have a test first written according to the requirement.\newline
\section{Make the proof of concept}
To show how test driven development can be done in a project we make a tiny project called 'Minesweeper' this game is calculating random mines which are placed in the field and if the user clicks a button assigned to have a mine the game is over. The project is defined in Java with a logic of a two dimensional array  and SWING as graphical interface. Here first the tests are defined and after that the functions are implemented. After done so there are Functional Tests and defined Interactive Tests.
\newpage
...
\end{enumerate}
\subsection{See how this works in our example with the mine-sweeper}
\begin{flushleft}
At first we need some utility functions containing the pure logic of the game. And in this place we first write a test which is written like the implementation of the class interface is already there. After doing so we make the real implementation.\newline Here at first the implemented test for the utility in the backend: 
\end{flushleft}
\begin{lstlisting}[language=Java]
   /**
...
\end{lstlisting}
\begin{flushleft}
The first test is the test for the method which places the mines in the field via a random algorithm. Here we test if the algorithm is really random and if the field is properly initialised.
\end{flushleft}
\begin{lstlisting}[language=Java]
 /**
...
\end{lstlisting}
\begin{flushleft}
The next test is the test for the method which builds a name of a button in the mine field. The point is that the name is used for saving coordinates and more. 
\end{flushleft}
\begin{lstlisting}[language=Java]
  /**
...
\begin{flushleft}
The next test is the test for the method which checks if a mine is hit by the button click 
\end{flushleft}
\begin{lstlisting}[language=Java]
 /**
...
\begin{flushleft}
The last test we show here is the test for the method which checks if the game end conditions for a positive end is done in the right way: 
\end{flushleft}
\begin{lstlisting}[language=Java]
/**
...
\end{lstlisting}
% now we go to the implementation of the functionality of the logic we have tests for:
\begin{flushleft}
After writing the tests for the public methods or interfaces we go ahead to implement the things for which we have written tests. To hold this example short I will show this for one method -> \abbrhighcol{THE PUBLIC METHOD FOR CALCULATE THE MINES IN THE FIELD}
\end{flushleft}

\begin{lstlisting}[language=Java]
  public List<List<ButtDescr>> calculateMines() {
  // first create the randomiser
  
        Random rand = new Random();
        int arrIndex = 0;
        
        // now we create the array with the dimensions set 
        ...
        // now we set the mines at first in the shadow array for plcing the values in the fields after that
       ...
       // place the shadow array values in the fields array
        arrIndex = 0;
        for (int cyInd = 0; cyInd < this.cy; cyInd++) {
            for (int cxInd = 0; cxInd < this.cx; cxInd++) {
...
\begin{flushleft}
In the example you see how the array is first created. After the creation we use a shadow array which is flattened to set the positions were mines are placed. This positions are after that placed in the fields array.\newline
This functionality can now be used by the GUI to build a mine field with mines which are placed random in the field. The other functions we shown in the test are after that used in the further logic.
\end{flushleft}


% Next chapter about test types and the categories
...
\end{flushleft}
\subsection{How to design a application for getting a clean testable project - in overview}
\begin{flushleft}
\abbrhighcol{There is more than one pattern to get an application clean with proper logic and a good testability:}\newline
The first pattern we will show is the \abbrhighcol{MVC} Model View Controler. \footnote{\abbrhighcol{MVC: } \abbrhighcol{M}odel \abbrhighcol{V}iew \abbrhighcol{C}ontroler is a model to separate the view (Maybe GUI) from the model which is the data model. And then use the controler to define the logic manipulating the data and build workflows for example} This pattern is very useful for standalone applications like a "big fat client" or something like Thunderbird (A mail program).Here a example:   \hyperref[ref:codelogic]{Model View Controller app example} \newline For services /  cloud computing the common sense is to think in a way which is a bit different from MVC. Let us have a look:\newline
Here we define separate layers in the application. In the following list represented from highest to lowest ->
\end{flushleft}
\begin{enumerate}
\item{\abbrhighcol{Business Logic: }In the business logic the implementation of the business part takes place. The business part ids the part with the logic according to the requirements and the view of the user. We avoid to implement backend things which do not belong to this logic because they are the technical realisation depending on the used systems.}
\item{\abbrhighcol{Backend: }Here in the technical part we have the view on the technical details dependent from the used products / systems and the internal data model. This part contains e.g. also select statements to access a specific database like MySQL or DB2 or MS Sql-Server.}
\item{\abbrhighcol{Separation: } The separation between the two parts is done through develop and define a API in form of interfaces which are implemented with several different backend 'processes' and so they decouple the one layer from the other completely.}
\end{enumerate}
\begin{flushleft}
Let us have a closer look on the structure of the backend so that we have a backend layer which allows clean testing.\newline
\newpage
\abbrhighcol{The backend itself:}
\end{flushleft}
\begin{tikzpicture}
% the nodes
\node [fill=green] (backA){\abbrhighcol{Database Layer}};
\node [fill=green,below=of  backA] (backB){\abbrhighcol{Transport Layer}};
\node [fill=green,below=of backB] (backC){\abbrhighcol{File System I/O}};
\node [fill=green,below=of backC] (backD){\abbrhighcol{Transport /Document SecurityLayer}};
\node [fill=green,below=of backD] (backE){\abbrhighcol{Configuration Layer - The detailed config}};
%drawings of vectors
\draw[->] (backA.south) -- (backB.north);
\draw[->] (backB.south) -- (backC.north);

\draw[->] (backC.south) -- (backD.north);

\draw[->] (backD.south) -- (backE.north);
\end{tikzpicture}
\begin{itemize}
\item{\abbrhighcol{Database Layer: }The database layer is a layer in which the general access to SQL / NoSQL (e.g.) databases is defined. Here the definition which is valid for all databases of a specific kind is done. }
\item{\abbrhighcol{Transport Layer : }In this layer the definition of the transport of data over different protocols takes place. Not only the mentioned but also cloud connectors and archive backends as a few examples can be implemented.}
\item{\abbrhighcol{File System I/O: }Here the I/O functionality to access file systems (directories, files) takes place.}
\item{\abbrhighcol{Transport /Document SecurityLayer: }In this layer the logic for a secure transport of data takes place as well as the logic to protect documents with hashes evidence records and signatures.}
\item{\abbrhighcol{Configuration Layer - The detailed config: }The config is stored in each project in a defined structure for example XML-BLOBS here in this layer the technical details how to create, update, read and delete the configuration or parts of it is defined and implemented.}
\end{itemize}
\newpage
\begin{flushleft}
\abbrhighcol{The interface to the backend and interfaces in the backend:}
\end{flushleft}
\rubric{\abbrhighcol{database layer}}
\begin{tikzpicture}
%% the circle
\draw  [fill=cyan](0,0) circle  (110pt);
\draw  [fill=red](0,0) circle  (30pt);
% the nodes
\node [fill=green] (backA){Under the database Layer};
\node [fill=green,above=of backA] (backB){Oracle access};
\node [fill=green,below=of backA] (backC){DB2 access};
\node [fill=green,left=of backA] (backD){MySQL access};
\node [fill=green,right=of backA] (backE){Postgres SQL access};
%drawings of vectors
\draw[->] (backA.north) -- (backB.south);
\draw[->] (backA.south) -- (backC.north);

\draw[->] (backA.west) -- (backD.east);
\draw[->] (backA.east) -- (backE.west);
\end{tikzpicture}\newline
\rubric{\abbrhighcol{transport layer}}
\begin{tikzpicture}
%% the circle
\draw  [fill=cyan](0,0) circle  (110pt);
\draw  [fill=red](0,0) circle  (30pt);
% the nodes
\node [fill=green] (backA){Under the transport Layer};
\node [fill=green,above=of backA] (backB){HTTP/HTTPS transport};
\node [fill=green,below=of backA] (backC){SFTP transport};
\node [fill=green,left=of backA] (backD){Azure Cloud transport};
\node [fill=green,right=of backA] (backE){Oracle Cloud Transport};
%drawings of vectors
\draw[->] (backA.north) -- (backB.south);
\draw[->] (backA.south) -- (backC.north);
...
\rubric{\abbrhighcol{File system I/O layer}}
\begin{tikzpicture}
%% the circle
\draw  [fill=cyan](0,0) circle  (110pt);
\draw  [fill=red](0,0) circle  (30pt);
% the nodes
\node [fill=green] (backA){Under the File I/O Layer};
\node [fill=green,above=of backA] (backB){NTFS access};
\node [fill=green,below=of backA] (backC){Journaled access};
\node [fill=green,left=of backA] (backD){Parallel NTFS access};
\node [fill=green,right=of backA] (backE){Parallel Journaled access};
%drawings of vectors
\draw[->] (backA.north) -- (backB.south);
\draw[->] (backA.south) -- (backC.north);
...
\rubric{\abbrhighcol{Transport /Document SecurityLayer - Following eIDAS}}
\begin{tikzpicture}
%% the circle
\draw  [fill=cyan](0,0) circle  (110pt);
\draw  [fill=red](0,0) circle  (30pt);
% the nodes
\node [fill=green] (backA){Under the Transport /Document SecurityLayer};
\node [fill=green,above=of backA] (backB){Secure Basic Auth};
\node [fill=green,below=of backA] (backC){Kerberos Auth};
\node [fill=green,left=of backA] (backD){Document Hashing};
\node [fill=green,right=of backA] (backE){Document Signing};
%drawings of vectors
\draw[->] (backA.north) -- (backB.south);
\draw[->] (backA.south) -- (backC.north);
...
\end{tikzpicture}\newline
\newpage
\rubric{\abbrhighcol{Configuration Layer - The detailed config}}
\begin{tikzpicture}
%% the circle
\draw  [fill=cyan](0,0) circle  (110pt);
\draw  [fill=red](0,0) circle  (30pt);
% the nodes
\node [fill=green] (backA){Under the Configuration Layer};
\node [fill=green,above=of backA] (backB){Config XML writer};
\node [fill=green,below=of backA] (backC){Config XML reader};
\node [fill=green,left=of backA] (backD){Config deletion};
\node [fill=green,right=of backA] (backE){Config XML update writer};
%drawings of vectors
\draw[->] (backA.north) -- (backB.south);
\draw[->] (backA.south) -- (backC.north);
...
\end{flushleft}
\subsection{How to design a application for getting a clean testable project - in detail (description)}
\begin{flushleft}
...
\abbrhighcol{let us have a look at the structure and the code to discuss it. }
\rubric{The high level project structure}

\begin{tikzpicture}
% the nodes
\node [fill=green] (dprA){\abbrhighcol{jsrc/main/ava}};
\node [fill=green,below=of  dprA] (dprB){\abbrhighcol{business}};
\node [fill=green,right=of dprB] (dprC){\abbrhighcol{bakend}};
\node [fill=green,right=of dprC] (dprD){\abbrhighcol{linklayer}};
\node [fill=green,below=of dprB] (dprE){\abbrhighcol{doc}};
\node [fill=green,right=of dprE] (dprF){\abbrhighcol{config}};
\node [fill=green,right=of dprF] (dprG){\abbrhighcol{config}};
%drawings of vectors
\draw[->] (dprA.south) -- (dprB.north);
\draw[->] (dprA.south) -- (dprC.north);
\draw[->] (dprA.south) -- (dprD.north);

\draw[->] (dprB.south) -- (dprE.north);
\draw[->] (dprB.south) -- (dprF.north);

\draw[->] (dprC.south) -- (dprG.north);
\end{tikzpicture}

\newpage
\end{flushleft}
\begin{flushleft}
\rubric{The code stubs for configuration to see the intention}
\rubric{The code of the interface}
\begin{lstlisting}[language=Java]
...
\end{lstlisting}
\rubric{The code of the config application class}
\begin{lstlisting}[language=Java]
...
public class AppConfig implements ConfigurationIfc {
 ...
 \end{lstlisting}
\rubric{The code of the config backend class}
\begin{lstlisting}[language=Java]
 ...
 */
public class Configuration implements ConfigurationIfc, AccessCtxAndConnIfc {
    /** HTTP port number*/
    private final int port;
 ...
 \end{lstlisting}
 \rubric{Generated config XML(JAXB class)}
\begin{lstlisting}[language=Java]
 ...
 *         &lt;element name="dbConnectString" type="{http://www.w3.org/2001/XMLSchema}string"/&gt;
 *         &lt;element name="defaultDocType" type="{http://www.w3.org/2001/XMLSchema}string"/&gt;
 *       &lt;/sequence&gt;
 *     &lt;/restriction&gt;
 *   &lt;/complexContent&gt;
 * &lt;/complexType&gt;
...
public class Appconfig {
    ...
    /**
     * Gets the value of the dbConnectString property.
     * 
     * @return
     *     possible object is
     *     {@link String }
    ...
    /**
     * Sets the value of the dbConnectString property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setDbConnectString(String value) {
...
\rubric{\abbrhighcol{The reasons of such a separation}}
\begin{flushleft}
The reason for making this separation is that the tests for separated components (separated by functionality) makes the different components better testable because the tests can be keeped simple in the way that we do not have to know in a test for a business object how the archive connector works and on the other side the archive connector is independent from the logic above.In this example we see that also Mocking Methods can be avoid better. It is a target to avoid mocking as far as ever possible although many people in our world tell we really need much mocking. Mocking a function is only ONE of MANY ways to solve a dependency problem and it does not make it really every time easier. I will take a deeper look on it. 
\end{flushleft}
\begin{flushleft}
The other important point is that the use of a new database or service is easier because there is only the last step which has to be implemented for that way with base functions which are defined in an interface. This way reduces errors and makes a great part of the software system independent. The advantage is that if you have a new Storage or Transmission with system dependencies you have only to test the implementation of the transport or storage interface and it is nearly all times the case that the rest of the product works well with it in cause of the strict separation via using a interface. Examples for this way are in Java the Collection and Stream and especially the InputStream / OutputStream and Reader / Writer interfaces.  
\end{flushleft}
\subsection{More about Mocking and 'Fake' Libraries / Services}
\rubric{\abbrhighcol{Mocking with Mockito or other mocking frameworks}}
\begin{flushleft}
...
\end{flushleft}
\rubric{\abbrhighcol{And here one thing that does not work without rewriting so that it can be mocked}}
\rubric{The app code}
\begin{lstlisting}[language=Java]
...
\end{lstlisting}
\rubric{The test code}
\begin{lstlisting}[language=Java]
@BeforeEach
    public  void beforeEach() {
        mockedVersion = Mockito.mock(MockedSample.class, Mockito.CALLS_REAL_METHODS);
    }
  @Test
...
\end{lstlisting}
\begin{flushleft}
\abbrhighcol{And calling this test leads with Java 17 leads to the following error message}
...
\end{flushleft}
\begin{flushleft}
Another problem is the count of calling methods inside a function of the application which is mocked. The count can be affected by e.g. someone changes the incoming data.
Here an example in which the check which can be broken very easy. Here an example:
 \rubric{The app code}
\begin{lstlisting}[language=Java]
...
\end{lstlisting}
\end{flushleft}
In the test code we see that the different count of the read functions has its reason in two different files. But the point is that there maybe the problem that anyone changes the test file without think about that a few lines more or less changes the outcome of the test.
One solution here can be:
 \begin{lstlisting}[language=Java]
   @Test
...
public void testMyFunction() throws Exception {
 ...
 \end{lstlisting}
 \abbrhighcol{Examples from stackoverflow and GitHub free to copy}
 \begin{flushleft}
...
\end{flushleft}
 \rubric{\abbrhighcol{'Fake' components}}
\begin{flushleft}
Fake components are components which have the same interface as the real component but do anything but returning values which are the right values to test the functionality. In the project / product I worked on the last years there was a component which was to expensive to buy it for tests only so we written a fake of the library doing reverse engineering and the component library we wrote was configurable for the parameters the return values and what should happen e.g. throw Exception XYZ. 
Let us use a interface working with Key Value Pairs and CSV. We have a 'Real World ' Implementation. In this example the advantages and disadvantages will be shown. Our tiny application has a 'library' part which is 'mocked away'.

\end{flushleft}
 \rubric{The real component}
 \rubric{The  component with the same interface ad returns for test reasons only} 
 \subsection{The top 10 NOT TODO's in tests}
\begin{flushleft}
...
\begin{itemize}
\item{Don't test against a only partly- or unknown target}
\item{Don't get too fancy when implementing a test keep the test simple}
\item{Do not check more than one aspect (parameter with different values, transaction with different kinds of input and result, ....) otherwise you do not know why exactly the test is failing }
\item{Don't use too much mocking if it is not necessary}
\item{Don't use a test description before you have double checked it if the description is not complete and proper the result cannot be good}
\item{Don't make tests which have no description at all}
\item{Don't be worried about ask the architect or developer questions about the functionality if you are not sure}
\item{A not to do is testing only the values like a developer would set them e.g. buffer-sizes for file transfer}
\item{Do not forget to test what happens if some things are not initialised correctly}
\item{Do not test like it is all times good weather and don't test in a nutshell (a unrealistic environment)}
\end{itemize}
\end{flushleft}
\newpage
...
\abbrhighcol{Now let us have a look at an old school dirty coded switch block}
\begin{lstlisting}[language=Java]
/**
...
\abbrhighcol{Now let us  have a look at an new  clean coded switch block}
\begin{lstlisting}[language=Java]
 /**
...
\end{lstlisting}
 ...
 \abbrhighcol{Now let us see how the switch expression works here the continuations are implicit and so coding which is too fancy is nearly impossible}
 \newpage
\begin{lstlisting}[language=Java]
   /**
...
%% next section
\subsection{Why the codestyle and coding rules and documentation standards support quality}
\begin{flushleft}
Not only the way a logic is developed gives a tribute to good quality but also the code style the coding rules and a check of unnecessary statements missing initialisations which can lead to unwanted effects are valuable for getting a good quality and good testable code.\newline
Imagine a team of ten developers. Each of the developers is new to the project. Before developing in that project each of the developers had a complete different history of projects with different formatting and coding rules and different ways of managing to get a good developed project. Now if all of these developers go on working as before you have completely different code styles in the project. And if the team gets bigger there are more and more different ways. Each way for itself will lead to a good result. But if you mix up all of them in one project any of the developers which is new to the project will face the problem to understand the logic and so on because he must spend much of the time he has to understand the different ways of development.\newline So if you start a project it is a good thing and a good way to define rules and a homogenous coding and development style and to set up development tools to support the defined rules. In case it is done in that way the project will be easier to understand by each one who is new to the project. Not also in coding but also in the documentation such standard definitions are very useful. \newline In the most cases there is no right and no wrong way except the technical needs of clean coding like initialise variables proper and active. So each project can have rules which differ slightly. But the main thing is to have these rules. This makes also documentation and code easier to read after a short time being in the project. And easier to read means better understanding and a consequence of that is normally a higher quality and a better testability.   
\end{flushleft}
\rubric{Tasks for aiming the goal}
\begin{itemize}
\item{Comment code - for publics in Java with Javadoc and in other languages with the best comment format}
\item{Format the code for each part in the same way. Here most Development tools have the possibility to store the formatting as settings and after they are stored you can do a reformat code.}
\item{Profile the code. Static code check e.g. via SonarCube which checks the code for unclean parts and also for ways of optimisation. This can also be processed during the build or in most Development Environments via plugin.}
\item{For the documentation the same tool like Word and there must be a standard how to write documentation for the different cases in the same way.}
\item{Also the way of defining the builds (e.g. with maven / the pom can be set up in different ways) and the modules has to be standardised.}
\end{itemize}
\begin{flushleft}
\rubric{Supporting tools}
Here some tools will be mentioned which help and support you doing development this way.\newline
At this point we cannot look at all the tools supporting this. There are many well known tools but also solutions for some things on GitHub and at GitHub they are available in source code. It is like with the test-frameworks like Cucumber which is available as source on GitHub. One source for the tools is the Wikipedia description of static code analysis. See \hyperref[ref:references]{The references Top 3.}
\end{flushleft}
\begin{itemize}
\item{\abbrhighcol{SonarCube:} Tool for static code analysis and to detect potential places for optimisation.here are many languages supported : C, C\#, C++, Objective-C, Kotlin, Java, Scala, JavaScript, TypeScript, VB.NET, Python, Cobol, PL/1, HTML to mention some of them}
\item{\abbrhighcol{CLang:} Code analysis for C / C++ / Objective-C}
\item{\abbrhighcol{Code Dx (Defunct 2021):} Code analysis for C, C++, C\#, Java JSP, Scala, Python}
\item{\abbrhighcol{ECLAIRE:} Code analysis C/ C++}
\item{\abbrhighcol{Fluid Attacks} Code analysis for C\#, Java, Kotlin and more}
\item{\abbrhighcol{GCC:} Code analysis for C complete analysis also for optimisation}
\end{itemize}
\rubric{Planning - Doing of Standards}
\begin{flushleft}
To introduce these standards to a project also the organisation of a project is very important. We cannot get standards alive in a project without planning and without having people like we say in German "wearing the hat" who decide if we use green as the colour for that specific point of logic or violet which are pictures for the fact that often neither the one nor the other opinion is the right way. There are many points where either the one or the other is ok. But there are also many points like code indention where the decision is a point which can be discussed for months or one makes the decision and it is ok no matter how the decision is.\newline
To make it short we need one or two persons who decide the things which will be otherwise points of discussions for a long time. So we need one who decides which way we choose and we need people who look that the coders developers ad the one who makes other parts of design and documentation and so on follows these decisions consequently.\newline Code review and the principal of four eyes have the same importance. \newline In that place let us construct an example:    
\end{flushleft}
\subsection{An example of planning and living these quality standards which help to better testing}
\begin{flushleft}
We have a project where we have to store CAD data for the construction of an airplane. This CAD data are processed via a C/C++ module to get them graphical  visible and this with the possibility to edit them. In addition to the CAD data we have a catalog of materials and parts we can use to construct the parts of the airplane like they are designed in the CAD. Now after that there is a workflow where a copy of the data is sent to thee responsible engineers and one copy of them is stored in parallel in an archive backend with a DMS / ECM written as a service in Java, JavaScript, Python and Scala.\newline
Since we have to manage the whole project we have to decide the coding standards for each language (Java, C/C+ ,Perl which is used for the batch generation, Scala, JavaScript,Python, and last but not least for the application service war ear and other packages.
\end{flushleft}
\rubric{Team structure top down non dynamic }
\begin{flushleft}
\end{flushleft}
\begin{tikzpicture}
% the nodes
\node [fill=green] (tA){\abbrhighcol{jTeam Lead / Personal Responsible}};
\node [fill=green,below=of  tA] (tB){\abbrhighcol{Project Architecture Board}};
\node [fill=green,below=of tB] (tC){\abbrhighcol{Development Team}};
\node [fill=green,right=of tC] (tD){\abbrhighcol{Test Team}};
\node [fill=green,right=of tD] (tE){\abbrhighcol{Delivery Support Team}};
%drawings of vectors
\draw[->] (tA.south) -- (tB.north);

\draw[->] (tB.south) -- (tC.north);
\draw[->] (tB.south) -- (tD.north);
\draw[->] (tB.south) -- (tE.north);
\end{tikzpicture}
\begin{flushleft}
\end{flushleft}
\rubric{Team structure dynamic nearly like Scrum}
\begin{flushleft}
\end{flushleft}
\begin{tikzpicture}
% the nodes
\node [fill=green] (tA){\abbrhighcol{jTeam Lead / Personal responsible}};
\node [fill=green,below=of  tA] (tB){\abbrhighcol{Development One}};
\node [fill=green,right=of tB] (tC){\abbrhighcol{Development Two}};
\node [fill=green,right=of tC] (tD){\abbrhighcol{Development Three}};
\node [fill=green,below=of tB] (tE){\abbrhighcol{Architecture Board}};
\node [fill=green,below=of tC] (tF){\abbrhighcol{Test Board}};
\node [fill=green,below=of tD] (tG){\abbrhighcol{Project Board}};
%drawings of vectors
\draw[->] (tA.south) -- (tB.north);
\draw[->] (tA.south) -- (tC.north);
\draw[->] (tA.south) -- (tD.north);

\draw[->] (tB.east) -- (tC.west);
\draw[->] (tC.west) -- (tB.east);

\draw[->] (tC.east) -- (tD.west);
\draw[->] (tD.west) -- (tC.east);

\draw[->] (tB.south) -- (tE.north);
\draw[->] (tC.south) -- (tE.north);
\draw[->] (tD.south) -- (tE.north);

\draw[->] (tB.south) -- (tF.north);
\draw[->] (tC.south) -- (tF.north);
\draw[->] (tD.south) -- (tF.north);

\draw[->] (tB.south) -- (tG.north);
\draw[->] (tC.south) -- (tG.north);
\draw[->] (tD.south) -- (tG.north);
\end{tikzpicture}

%Comment
\begin{flushleft}
As you see in the graphical view the organisation of the first structure looks very static and hierarchical as it was long times in former times.\newline The new agile organisation looks, except the team lead who has here a more and more pure personal responsibility, like a net which is a more dynamic structure and formation. This leads to a flexible structure of \abbrhighcol{SCRUM Teams} and out of the Scrum teams there are some people, in most cases the Scrum Masters, who have the one or other steering and design function. This is interesting for Quality and testing because the exchange of information is much higher.\newline But you see also that there is a test team outside the hierarchy. It is outside because it needs to be independent. This fact that the test team is not responsible against the project lead or any others leads to better testing and higher quality.   
\end{flushleft}


%% New part and chapture
\newpage
\chapter{Further material}
\section{Code examples and logic}
\label{ref:codelogic}
\begin{flushleft}
In this part some code snippets and explanations take place which were only mentioned in the article.
Here the list:
\begin{enumerate}
\item{The Model View Controller Architecture}
\end{enumerate}
\end{flushleft}
\subsection{The MVC Model as clean separation of functionality}
\label{ref:mvc}
\begin{flushleft}
\end{flushleft}
\section{References}
...
\item{\href{https://en.wikipedia.org/wiki/List_of_tools_for_static_code_analysis}{Static code analysis description Wikipedia - Tools - Table}}
...
\item{\href{https://ryanclements.dev/posts/you-probably-dont-need-mocking}{A interesting article about mocking.... Is it really useful to mock all ??? ;-)}}  
\end{enumerate}
\end{document}